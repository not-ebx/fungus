...
use crate::login_packets::login_packets::on_check_password_result;

pub async fn handle_check_login_auth_info(session: &mut ClientSession, in_packet: &mut InPacket) -> Result<OutPacket, PacketError> {
    ...
    let user_result = User::get_user_by_username(
        username.clone()
    );
    ...
    let packet = match user_result {
        ...
        _ => {
            ...
            if ALLOW_AUTO_REGISTER {
                new_user = Some(User::insert_new_user(
                    username.clone(),
                    password.clone()
                ).unwrap());
