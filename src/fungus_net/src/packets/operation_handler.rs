...
use crate::packets::login_packets::handlers::login_handlers::{handle_char_select_no_pic, handle_check_duplicate_id, handle_check_login_auth_info, handle_create_new_character, handle_select_world, handle_world_list_request, handle_world_status_request};
...
pub async fn handle_packet(session: &mut ClientSession, mut packet: InPacket) -> Result<(), PacketError> {
    ...
    match opcode {
        ...
        InHeader::CharSelectNoPic => {
            handle_char_select_no_pic(session, &mut packet).await
        }
        /*
            InHeader::GuestLogin => {}
