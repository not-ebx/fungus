...
pub trait CreateGameItem<T> {
    async fn create_item(&self, tx: &mut Transaction<'_, Postgres>, item: &mut T) -> Result<(), sqlx::Error>;
}
...
impl CreateGameItem<ItemSerializer> for ItemDAO {
    async fn create_item(&self, tx: &mut Transaction<'_, Postgres>, item: &mut ItemSerializer) -> Result<(), sqlx::Error>{
...
impl CreateGameItem<EquipmentSerializer> for ItemDAO {
    async fn create_item(&self, tx: &mut Transaction<'_, Postgres>, item: &mut EquipmentSerializer) -> Result<(), Error> {
        sqlx::query!(
