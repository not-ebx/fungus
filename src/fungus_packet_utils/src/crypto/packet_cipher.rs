...
impl PacketCipher {
    pub fn new(siv: [u8; 4], riv: [u8; 4]) -> Self {
        ...
        let s_version =
            (((0xFFFF - g_version) >> 8) & 0xFF) | (((0xFFFF - g_version) << 8) & 0xFF00);
        ...
        let send_cipher = Ofb::<Aes256>::new_from_slices(&AES_KEY, &new_siv).expect("send ded");
        ...
        let recv_cipher = Ofb::<Aes256>::new_from_slices(&AES_KEY, &new_riv).expect("recv ded");
    ...
    pub fn crypt(&mut self, buffer: &mut [u8], iv: &[u8]) {
        ...
        while remaining > 0 {
            let mut aes_ofb = Ofb::<Aes256>::new_from_slices(&AES_KEY, iv)
